...
class KJPEG:
    def __init__(self, quality_scale):
        # 压缩率
        self.__quality_scale = quality_scale
        # 初始化DCT变换的A矩阵 __dctA
        ...
        if (self.__quality_scale <= 0):
            self.__quality_scale = 1
        if (self.__quality_scale >= 100) :
            self.__quality_scale = 99
        for i in range(0, 64):
            ...
            temp = (self.__cq[i] * self.__quality_scale + 50) / 2
            if temp <= 0: tmep = 1
            if temp >= 0XFF: temp = 0XFF
            self.__cq[i] = temp
    ...
    def Compress(self, filename):
        ...
        buff = 0
        tfile = os.path.splitext(filename)[0] + str(self.__quality_scale) + ".txt"
    ...
    def Decompress(self, filename):
        ...
        image.save("./result"+str(self.__quality_scale)+".bmp", "bmp")
...
if __name__ == '__main__':
    ...
    kjpeg.Decompress("./lenna20.txt")

    kjpeg = KJPEG(10)
    kjpeg.Compress("./lenna.bmp")
    kjpeg.Decompress("./lenna10.txt")

    kjpeg = KJPEG(5)
    kjpeg.Compress("./lenna.bmp")
    kjpeg.Decompress("./lenna5.txt")

    kjpeg = KJPEG(1)
    kjpeg.Compress("./lenna.bmp")
    kjpeg.Decompress("./lenna1.txt")